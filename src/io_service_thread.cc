        ...
        : _work(_io_service)
{
        for (size_t i = 0; i < size; ++i) {
                ...
                boost::thread::id t_id = _workers.create_thread(
                        boost::bind(&boost::asio::io_service::run, &_io_service))
                        ->get_id();
//                thread_https_pool[t_id] = http;
                //_thread_https_pool.insert(pair<boost::thread::id, SrNetHttp*>(
                //                                        t_id, http
                //                                 ));
        ...
        _thread_https_pool[t_id] = new SrNetHttp(agent.server() + "/s", srv, agent.auth());
}
...
//        _acceptor.listen(60000, ec);
        if (ec) {
                SSADebug("acceptor listen error");
        }
...
void IOServiceThread::StartHandleAccept()
{
        ...
        session_ptr new_session(new ClientSession(_agent, _tp.getIoService(),
