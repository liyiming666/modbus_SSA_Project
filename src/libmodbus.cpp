...
int Modbus::readBat(const int& addr, const int& nb, u8* req, int func_code)
{
...
{
 	int ret = _ctx->backend->build_request_basis(
                _ctx, MODBUS_FC_WRITE_SINGLE_COIL, num-1, nb?0xFF00:0, req);
...
{
 	int ret = _ctx->backend->build_request_basis(
		_ctx, MODBUS_FC_WRITE_SINGLE_REGISTER, num-1, nb, req);
        ret =  _ctx->backend->send_msg_pre(req, ret);
        this->print(req, ret);
